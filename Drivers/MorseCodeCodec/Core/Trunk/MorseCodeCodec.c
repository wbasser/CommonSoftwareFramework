...
// library includes -----------------------------------------------------------
#if ( MORSECODECODEC_ENABLE_DEBUG == ON )
...
static  S16               sCurRcvDotElementCount;
static  U8                nCurRcvWpm;

// local function prototypes --------------------------------------------------
static  C8    CodeToAscii( PMORSEECODECODEDEF ptCode );
...
void MorseCodeCodec_Config( BOOL bState )
{
  ...
  if ( ON == bState )
  {
    ...
    lTimeBase = 1000000 / lTimeBase;
    sCurRcvDotElementCount = lDotTimeUsec / lTimeBase;
    #if ( MORSECODECODEC_ENABLE_DEBUG == ON )
      DebugManager_AddElement( 0xBFFF, sCurRcvDotElementCount );
    #endif
  }
}
  ...
  else
  {
    ...
    switch( eXmtCtlState )
    {
      ...
      case XMT_CTL_STATE_KEYDN :
        ...
        // set the time for inter element time
  ...
  switch( eRcvCtlState )
  {
    ...
    case RCV_CTL_STATE_KEYDN :
        ...
        if ( MORSECODE_EDGE_KEYUP == tData.eEdge )
        {
          ...
          // deteremine dash/dot
          if ( tData.wValue > ( sCurRcvDotElementCount << 1 ))
          {
            // dash element
          ...
          // adjust
          sCurRcvDotElementCount = tData.wValue * 0.95;
        
          // increment the length
          tCurRcvCode.tFields.nLength++;
        
          #if ( MORSECODECODEC_ENABLE_DEBUG == ON )
            DebugManager_AddElement( 0xC200, sCurRcvDotElementCount );
          #endif
          ...
          // recalculate the WPM
          nCurRcvWpm = sCurRcvDotElementCount / WPM_BASE_TIME_MSEC;
          #if ( MORSECODECODEC_ENABLE_DEBUG == ON )
