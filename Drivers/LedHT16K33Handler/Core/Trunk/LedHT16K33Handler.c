...
static  U8    anDisplayBuffer[ MAX_NUM_DIGITS * 2 ];
 ...
 *
 *****************************************************************************/
void LedHT16K33Handler_Initialize( void )
{
  ...
  // execute it
  LedHT16K33Handler_WriteData( tCmd.nByte, NULL, 0 );
}
 ...
 *
 *****************************************************************************/
void LedHT16K33Handler_Clear( void )
{
  // clear the display
  memset( &anDisplayBuffer, 0, ( MAX_NUM_DIGITS << 1 ));
  LedHT16K33Handler_UpdateDisplay( );
}
 ...
 *
 *****************************************************************************/
void LedHT16K33Handler_SetBlink( LEDHT16K33BLINKRATE eBlinkRate )
{
  LEDHT16K33CMDDEF  tCmd;
 ...
 *
 *****************************************************************************/
void LedHT16K33Handler_SetBrightness( LEDHT16K33BRIGHTNESS eBrightness )
{
  LEDHT16K33CMDDEF  tCmd;
 ...
 *
 *****************************************************************************/
void LedHT16K33Handler_DisplayColon( BOOL bDisplayColon )
{
  // check for colon
  if ( bDisplayColon == TRUE )
  {
    // turn on cursor
    anDisplayBuffer[ COLON_BUF_INDEX << 1 ] = 0x02;
  }
}
 ...
 *
 *****************************************************************************/
void LedHT16K33Handler_DisplayChar( U8 nDigit, U8 nCharacter, BOOL bDecimal )
{
  U8  nBitmap, nBufIndex;

  // adjust for colon
  if ( nDigit >= COLON_BUF_INDEX )
  {
    // adjust digit
    nDigit++;
  }
  ...
  // store the character
  anDisplayBuffer[ nBufIndex ] =  nBitmap;
}
 ...
 *
 *****************************************************************************/
void LedHT16K33Handler_DisplayNumber( U8 nBase, U32 uValue, U8 nDecimalPointLoc, BOOL bDisplayColon )
{
      ...
      (( 16 == nBase ) && ( uValue <= LEDHT16K33_MAX_NUMBER_BASE_HEX )))
  {
    ...
    nCurDigit = LEDHT16K33_NUM_DIGITS;
    ...
    while ( uValue > 0 )
    {
      ...
      // display the digit
      LedHT16K33Handler_DisplayChar( nCurDigit - 1, nCurValue, ( nDecimalPointLoc == nCurDigit ));
  ...
  {
    // for each character 
    for ( nCurDigit = 0; nCurDigit < LEDHT16K33_NUM_DIGITS; nCurDigit++ )
    {
      // display a dash
      LedHT16K33Handler_DisplayChar( nCurDigit, LEDHT16K33_SPCCHAR_MINUS, OFF );
    }
  }
 ...
 *
 *****************************************************************************/
void LedHT16K33Handler_UpdateDisplay( void )
{
  LEDHT16K33CMDDEF  tCmd;
  ...
  tCmd.tFields.nOpt = 0;
  LedHT16K33Handler_WriteData( tCmd.nByte, anDisplayBuffer, ( MAX_NUM_DIGITS << 1 ));
}
